...
while ($row_product = mysqli_fetch_assoc($product_result)) {
    ...
    $status = $row_product['status']; // Retrieve order date from the database
    ...
    echo "$name ($email and the retailer's phone number is: $phone) ordered $item, $quantity quantities, gender $gender, size $size. Shipping address: $shipping_address. The payment method is $payment_method, and the shipping method is $shipping_method. Order placed on: $order_date. Total Price: $total_price. Status: $status";
